...
<div class="border-top border-bottom main-content margin-below-sticky-header stripe-bg">
    <div class="flex-container page-header">
        <div class="flex-66">
            <div class="heading-container-left small-page-heading"><h1 class="center-on-mobile"> <?= _MAGAZINE_NAME?></h1></div>
        </div>
        <div class="flex-33 breadcrumbs ">
            <p class="desktop-only"><a href="<?= $ariacms->actual_link ?>"><?= _HOME ?></a> / Số tạp chí</p>
        </div>
    </div>
</div>

<div class="white-band "
     style="padding: 25px 0 100px 0; background-image: linear-gradient(90deg, #ffffff 49.75%, #fafafa 49.75%, #fafafa 50%, #ffffff 50%, #ffffff 99.75%, #fafafa 99.75%, #fafafa 100%); background-size: 402.00px 402.00px;">
                    ...
                    <?php foreach ($news as $key => $new) {
                        ...
                        <div class="flex-col-portfolio-page">
                            <div class="project-container">
                                <a href="<?= $ariacms->actual_link ?>chu-de/<?= $new->url_part ?>.html">
                                    <div class="project-image-container">
                                        <picture class="attachment-full size-full">
                                            <source type="image/webp" data-lazy-srcset="<?= $new->image ?>"
                                                    srcset="<?= $new->image ?>"/>
                                            <img width="765" height="430" src="<?= $new->image ?>" alt=""
                                                 data-lazy-srcset="<?= $new->image ?>"
                                                 data-lazy-sizes="(max-width: 765px) 100vw, 765px"
                                                 data-lazy-src="<?= $new->image ?>"/>
                                        </picture>
                                        <noscript>
                                            <picture class="attachment-full size-full">
                                                <source type="image/webp" srcset="<?= $new->image ?>"
                                                        sizes="(max-width: 765px) 100vw, 765px"/>
                                                <img width="765" height="430" src="<?= $new->image ?>" alt=""
                                                     srcset="<?= $new->image ?>" sizes="(max-width: 765px) 100vw, 765px"/>
                                            </picture>
                                        </noscript>
                                    </div>
                                </a>
                                <div class="client-name-container"><a class="client-name"
                                                                      href="<?= $ariacms->actual_link ?>chu-de/<?= $new->url_part ?>.html"><?= $new->{$params->title} ?></a>
                                    <!-- <p class="city"><?= $ariacms->unixToDate($new->post_created, '/') ?></p> -->
                                </div>
                            </div>
                        </div>
