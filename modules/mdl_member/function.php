...
class Model
{
			...
			if($tieude !=""){
				...
				if($id > 0){
					...
					';
						...
						$subject3 = "Cảm ơn tác giả gửi bài báo về Tạp chí IBST";
						$content3='
						<p>Tạp chí KHCN Xây dựng – Viện KHCN Xây dựng đã nhận được bài báo tác giả gửi.<br> Ban biên tập sẽ xem và phản hồi trong thời gian sớm.<br>
						Trân trọng cảm ơn.</p>
					
					';
						// Gửi email cho khách hàng
						if($ariacms -> sendMail( $_SESSION['member']['email'] , $_SESSION['member']['fullname'],$subject3,$content3)){
							//sendMail($to_email,$to_name,$subject,$content);
							//redirect("Đăng ký thành công, vui lòng xác nhận tài khoản qua email đăng ký của bạn","/");
							//echo '<script language="javascript">alert("Đăng ký thành công, vui lòng xác nhận tài khoản qua email đăng ký của bạn.");</script>';
						}
					
					echo '<script language="javascript">alert("Bài báo của tác giả đã gửi thành công.
                                                                   Trân trọng cảm ơn!");window.location.href ="/member/bai-viet.html";</script>';
				}else{
	...
	function quanlyphanbien(){
		...
		$query = "SELECT c.*, b.* FROM e4_users a 
                        ...
                        WHERE b.id_nguoiphanbien = $member_id AND b.trangthai !=0  order by b.id";
		$database->setQuery($query);
	...
	function phanbien(){
        ...
        if($_POST){
            ...
            $row->noidungphanbien 	= $_POST['noidungphanbien'];
            $row->trangthai     = 3;
            $row->file     =  $_POST['file'];
            $row->time 	= time();
            $row->ngaypb 	= time();
            ...
            if($id_baibao > 0){
                ...
                foreach($admin as $ad){
                    ...
                    // 	echo '<script language="javascript">alert("Gửi email thất bại.");</script>';
                    // }
					$subject3 = "Cảm ơn phản biện bài báo về Tạp chí IBST";
						$content3='
						<p>Tạp chí KHCN Xây dựng – Viện KHCN Xây dựng đã nhận được bài phản biện gửi. Ban biên tập sẽ xem và phản hồi trong thời gian sớm.
						Trân trọng cảm ơn.</p>
	...
	function capnhatbaiviet()
    {
        ...
        if ($member_id > 0 and $_SESSION['member']['publish'] == 1) {
            if ($tieude != "") {
                if ($status == 'phanhoitacgia' and $_POST['submit'] == 'phanhoitacgia') {
					...
					';
                            ...
                            if ($ariacms->sendMail($ad->email, $ad->fullname, $subject2, $content2)) {
								...
								$subject3 = "Cảm ơn tác giả gửi bài báo về Tạp chí IBST";
								$content3='
								<p>Tạp chí KHCN Xây dựng – Viện KHCN Xây dựng đã nhận được bài báo tác giả gửi. Ban biên tập sẽ xem và phản hồi trong thời gian sớm.
								Trân trọng cảm ơn.</p>
    ...
    public static function dongy()
    {
        ...
        $query = "SELECT count(id) as count FROM `e4_phanbien` WHERE id = ". $id . " and mabaibao = '".$mabaibao."' and trangthai > 0";
        $database->setQuery($query);
        $check = $database->loadRow();
		$check = $check['count'];
		//echo $check['count'];die;
        if($check > 0){
            ...
            if($id_baibao){
                ...
                $subject2 = "Tạp chí Khoa học công nghệ xây dựng – Viện KHCN Xây dựng cảm ơn";

                $content2='
						<p>Tạp chí Khoa học công nghệ xây dựng – Viện KHCN Xây dựng xin cảm ơn bạn đã chấp nhận lời mời phản biện</p>
						<p>Thời gian phản biện: trong 15 ngày kể từ ngày chấp nhận.</p>
						<p>Thông tin chi tiết hiện luôn ra trang nội dung cần góp ý.</p>
        ...
        if($check){
            ...
            if($id_baibao > 0){
                ...
                foreach($admin as $ad){
					...
					$content2='
						<p>Cảm ơn bạn</p>
						<p>Hi vọng bạn sẽ chấp nhận lời mời phản biện cho các bài báo khác.</p>
						';
					// Gửi email cho khách hàng
					if($ariacms -> sendMail( $_SESSION['member']['email'] , $_SESSION['member']['fullname'],$subject2,$content2)){
						echo '<script language="javascript">alert("Cảm ơn bạn đã đồng ý. Vui lòng kiểm tra mail.");window.href="'.$ariacms->actual_link.'"</script>';
					}
                }
