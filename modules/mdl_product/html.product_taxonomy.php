			...
			<div class="row">
				<div class="col-lg-12">
					<div class="breadcrumb__links">
						...
						<a href="<?= $ariacms->actual_link ?>san-pham.html"><?= _PRODUCT_LIST ?></a>
						<span><?= $category[$params->title] ?></span>
					</div>
				</div>
			</div>
		</div>
								...
								<div class="accordion" id="accordionExample">
									...
									foreach ($taxonomies as $taxonomy) {
										if ($taxonomy->taxonomy == 'product_category' && $taxonomy->submenu > 0 && $taxonomy->parent == 0) {
											$active = (in_array($taxonomy->id, explode(',', $category['submenu']))) ? ' active' : '';
											$show = (in_array($taxonomy->id, explode(',', $category['submenu']))) ? ' show' : '';
											foreach ($taxonomies as $taxonomy_sub) {
												if ($taxonomy->id == $taxonomy_sub->parent) {
													$active = (in_array($taxonomy_sub->id, explode(',', $category['submenu']))) ? ' active' : $active;
													$show = (in_array($taxonomy_sub->id, explode(',', $category['submenu']))) ? ' show' : $show;
												}
											}
									?>
											<div class="card">
												...
												<div id="collapse<?= $taxonomy->id ?>" class="collapse <?= $show ?>" data-parent="#accordionExample">
													<div class="card-body">
														<ul>
															<?php
															foreach ($taxonomies as $taxonomy_sub) {
