...
if($_SESSION['user']['role_type'] == 'ADMIN' || $_SESSION['user']['role_type'] == 'CHECK'){
	...
	$query = "SELECT * FROM e4_posts a WHERE a.id =".$_REQUEST['news_id'];
	...
	$query = "update e4_posts set post_status= '".$post_status."' , user_aproved=".$_SESSION["user"]['id']." where id= ".$_REQUEST['news_id'];
	$database->setQuery($query);
	$database->query();
	echo $query;
}
