...
class Model
{
	...
	static function news_view()
	{
		...
		/**/
		$query = "SELECT COUNT(id) total FROM e4_posts a WHERE a.post_type = 'post'".$where;
		$database->setQuery($query);
	...
	static function news_publish()
	{
		...
		*/
		...
		FROM e4_posts a
			LEFT JOIN e4_users b ON a.user_created = b.id 
			LEFT JOIN ( 
				SELECT t1.object_id, t2.title_vi AS category, t2.id as category_id
				FROM e4_term_relationships t1 
				LEFT JOIN e4_term_taxonomy t2 ON t1.term_taxonomy_id = t2.id AND t2.taxonomy = 'category' GROUP BY t1.object_id
				) c ON a.id = c.object_id
		WHERE a.post_status = 'active' " . $where . $order . $limit;
	...
	static function news_add()
	{
		...
		if ($_POST["submit"] != "") {
			//print_r($_POST);
			//print_r($_FILES);die;
			//echo $_POST['image_thumb'];die;
			$targetDir = "../upload/advertisement/";
			...
			if($_POST['image_thumb']){
				$images = $_POST['image_thumb'];
			}
			if($_FILES['hinhanh']['name']){
				...
				$image_link = $ariacms->actual_link ."upload/advertisement/".$fileName;
			...
			$link = "index.php?module=news&tab=home";
			//if($_SESSION['user']['role_type'] !="ADMIN"){
				if ($_REQUEST["submit"] == "xuatban") { // Gỡ bài đăng
					...
					$link = "index.php?module=news&tab=menu4";
				}
				//$row->post_status = 'waiting';
			//}
			$row->post_type = 'post';
			$row->meta_title = $_POST['title_vi'];
			$row->aproved_date = strtotime(str_replace('/','-',$_POST['aproved_date']));
	...
	static function news_edit()
	{
		...
		if ($_REQUEST["submit"] != "") { // Cập nhật bài viết
			...
			$link ="index.php?module=news";
			if($_SESSION['user']['role_type'] == "ADMIN"){
				if ($_REQUEST["submit"] == "gobaidang") { // Gỡ bài đăng
					$link = "index.php?module=news&tab=menu3";
					...
					$link = "index.php?module=news&tab=menu1";
					...
					$link = "index.php?module=news&tab=menu2";
				}else if ($_REQUEST["submit"] == "tralai") { // Từ trả lại
					$row->post_status = 'tralai';
					$link = "index.php?module=news&tab=menu5";
				}
