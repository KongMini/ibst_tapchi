...
global $database;
		...
		$array_location = array(2=> 'Tin TOP trang chủ', 1=>'Nổi bật bên phải');
?>
				...
				<div class="box-body table-responsive">
					<form method="get" action="index.php" name="form_news_search" id="form_news_search">
						...
						<div class="form-group">
							...
							<div class="col-md-2">
								<input type="submit" class="btn btn-success" name="timdulieu" value="Tìm kiếm" />
							</div>
						</div>
					</form>
					<div class="clear"></div>
							...
							<table id="table-2" class="table table-bordered table-hover">
								...
								<tbody>
									...
									foreach ($news as $new) {
										$i++;
									?>
									<?php if($new->news_position > time()){ $title_post = "<i style='color: #ec3e3e'>".$new->title_vi."</i>";$color = "color: #ec3e3e";}else{$title_post = $new->title_vi;$color = "";} ?>
										<tr id="<?php echo $new->id;?>" class="<?php echo ($i % 2 == 1) ? 'bg-gray-light' : ''; ?> valign-middle" <?php if($new->news_position == 2) {echo 'style="background: rgb(182, 233, 228);"';} else if($new->news_position == 4){echo 'style="background: #d5d5fb;"';} ?>>
											<td  class="text-center">
												<?php echo $i ?>
											</td>
											<td><?php echo '<a href="javascript:void(0);" data-toggle="modal" data-target="#showCNTT"  onclick="showCNTT(\'' . $new->id . '\',\'ajax/news/ajax.news_get.php\')">'.$title_post.'</a>' ?></td>
