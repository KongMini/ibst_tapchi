...
class Model
{
	...
	static function news_view()
	{
		...
		// điều kiện 4:thao tác
		if(isset($_REQUEST['action'])){
			$action = $_REQUEST['action'];
			if($action !== ""){
				//echo $vitri; die;
				$where4 = " and a.action = '".$action."'";
			}
		}
		...
		if($_REQUEST['user_created']){
			...
			if($user_created != ""){
				$where6 = " and b.id = '".$user_created."'";
			}	
		}
	
		$where = $where4.$where3.$where6 ;
		//===================END====================
		...
		$order = ' order by a.post_modified desc';
		...
		FROM e4_posts_history a
			LEFT JOIN e4_users b ON a.user_created = b.id where 1 = 1 ".$where . $order. $limit;
		//WHERE 1 = 1 " . $where . $order . $limit;
		
		$database->setQuery($query);
		$news = $database->loadObjectList();
		
		$query = "SELECT COUNT(*) total,a.*, b.fullname
		FROM e4_posts_history a
			LEFT JOIN e4_users b ON a.user_created = b.id WHERE 1 = 1 ".$where;
